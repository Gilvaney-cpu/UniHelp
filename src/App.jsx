...
export default function App() {
  ...
  const handleSendMessage = async (text) => {
    ...
    setChatHistory(prev => { const updated = [...prev, newUserMessage]; saveChat(updated); return updated; });
    setIsLoading(true);
  ...
  const handleValidationResponse = async (id, response) => {
    if(db) try { await addDoc(collection(db, "validacoes_fatos"), { fatoId: id, confirmacao: response, data: new Date() }); } catch(e) {}
